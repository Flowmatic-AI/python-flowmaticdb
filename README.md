...
`db.pubsub` is fanout publish/subscribe that behaves the same on all three
engines — intended for pushing websocket messages out across a gateway.
...
for message in subscription.messages(timeout=1.0):
    print(message.channel, message.payload)
```

`pubsub` is a cached property, not a method: it owns threads and a connection
and has to be closed, so there is exactly one per database and `db.close()`
tears it down. Building it is cheap — no thread starts and no connection opens
until the first `subscribe()`.

### One API, three mechanisms

The backend is chosen by dialect. A caller cannot tell which one delivered.

| Engine | Backend | Mechanism | Latency | Standing cost |
|---|---|---|---|---|
| PostgreSQL | `PostgresPubSub` | native `LISTEN`/`NOTIFY` | sub-millisecond | one dedicated connection |
| MySQL / MariaDB | `PollingPubSub` | outbox table, polled | ~75–150ms | `processes / poll_interval` queries per second |
| SQLite | `MemoryPubSub` | in-process broker | microseconds | none |

The polling backend needs its table created explicitly, like migrations:

```python
db.pubsub.init()   # PollingPubSub only
```

### What every backend guarantees

These are the portability contract. Anything an engine cannot do natively is
constrained down to the common denominator rather than exposed unevenly, so
code written against SQLite runs unchanged on PostgreSQL.

- **Channel names** are 1–63 characters of `[a-z0-9_]`. That is PostgreSQL's
  identifier limit, enforced everywhere.
- **Payloads** are `str`, at most 7999 bytes *encoded*. That is PostgreSQL's
  `NOTIFY` limit, enforced everywhere. Encoding is yours to choose.
- **Delivery is at-most-once**, ordered within a channel, unordered across
  channels. A subscriber that was not connected does not get history — websocket
  clients resync on reconnect, which is what makes this acceptable.
- **A slow consumer drops messages rather than blocking.** Each subscription has
  a bounded queue that discards the oldest first and counts it in
  `subscription.dropped_count`. Blocking would stall the thread feeding every
  other subscriber.

### Publish after the transaction, not inside it

Publishing is fire-and-forget and should be its own statement, issued *after*
the business transaction commits:

```python
db.transaction(lambda database: database.insert("messages").values(row).execute())
db.pubsub.publish("room_42", payload)
```

You can then lose a notification by crashing between the two. That is the
intended trade: the alternative — publishing inside the transaction — widens the
polling backend's grace window from tens of milliseconds to the length of your
longest write transaction, and clients resync on reconnect anyway.

### Consuming from asyncio

The library is synchronous. A websocket server is not, so the poller runs in its
own thread and hands messages over:

```python
subscription = db.pubsub.subscribe(["room_42"])

def pump(loop, client_queue):
    for message in subscription.messages(timeout=1.0):
        loop.call_soon_threadsafe(client_queue.put_nowait, message.payload)
```

`subscription.poll()` is the non-blocking alternative when you would rather
drain on your own schedule than park a thread. Note that a thread running a
poll loop holds a connection and counts against `max_concurrent_connections`.

### Two things that will bite

**Forking.** `MemoryPubSub` is correct only while publisher and subscriber share
a process. Under `uvicorn --workers 4` each worker would get its own broker and
cross-worker messages would vanish. It records its pid and raises `PubSubError`
rather than losing them silently — but single-worker-with-threads is the
deployment it assumes.

**High-frequency ephemeral traffic.** Typing indicators, cursor positions and
scroll sync are worthless a second later and will hammer the outbox for nothing.
Keep them off the polling backend.

### Tuning

Via the `options` dict on any `connect_*()`:

| Option | Default | Applies to |
|---|---|---|
| `pubsub_backend` | by dialect | all |
| `pubsub_max_queued_messages` | `1000` | all |
| `pubsub_reconnect_interval` | `1.0` | PostgreSQL |
| `pubsub_table` | `"pubsub_messages"` | polling |
| `pubsub_poll_interval` | `0.1` | polling |
| `pubsub_grace_milliseconds` | `50` | polling |
| `pubsub_retention_milliseconds` | `300000` | polling |
| `pubsub_run_janitor` | `True` | polling |
| `pubsub_janitor_interval` | `30.0` | polling |

`pubsub_backend` overrides the dialect's choice. It is a `StrEnum`, so a member
and its plain string are interchangeable — which matters when options come from
configuration — and a misspelling is refused rather than quietly falling back:

```python
from flowmaticdb.pubsub import PubSubBackendEnum

db = DB.connect_sqlite("app.db", options={"pubsub_backend": PubSubBackendEnum.POLLING})
db = DB.connect_sqlite("app.db", options={"pubsub_backend": "polling"})   # same thing
```

This is mainly for a **custom dialect and adapter**, where the library has no
basis for guessing. It defaults to `polling` there, deliberately: polling is the
only backend that delivers between processes on an engine this library has never
seen, and it fails loudly when a dialect cannot express its SQL. `memory` never
fails and never crosses a process boundary — a silent hole in any deployment
running more than one worker — so it is available by name rather than by
default. Choose it when you know your deployment is single-process.

You can also construct a backend directly and own its lifecycle:

```python
from flowmaticdb.pubsub import PollingPubSub

pubsub = PollingPubSub(db, poll_interval=0.05)
pubsub.init()
```

### What this is not

Fanout, not a work queue: every subscriber sees every message. Competing
consumers that each handle a message once need `FOR UPDATE SKIP LOCKED`, which
SQLite has no equivalent for — a different design.
...
└── PubSubError         — Pub/sub validation and lifecycle errors
```
...
| `GENERATED BY DEFAULT AS IDENTITY` | ✅ | Opt-in via `use_serials: False` (≥ 17); auto-increment columns are `SERIAL`/`BIGSERIAL` by default |
