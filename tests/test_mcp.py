...
def server() -> FlowmaticDBMCP:
    ...
    return FlowmaticDBMCP(db, "flowmaticdb-test")


@pytest.fixture
def grouped() -> FlowmaticDBMCP:
    db = DB.connect_sqlite(":memory:")
...
def test_every_capability_is_registered_as_a_tool(server: FlowmaticDBMCP) -> None:
    tools = asyncio.run(server.server.list_tools())

    assert {tool.name for tool in tools} == TOOL_NAMES


def test_tools_are_callable_over_the_protocol(server: FlowmaticDBMCP) -> None:
...
def test_select_arguments_validate_over_the_protocol(server: FlowmaticDBMCP) -> None:
...
def test_driver(server: FlowmaticDBMCP) -> None:
    assert server.driver() == "sqlite"


def test_execute_sql_returns_rows(server: FlowmaticDBMCP) -> None:
    rows = server.execute_sql("SELECT name FROM users WHERE age > ? ORDER BY age", [25])

    assert rows == [{"name": "Alice"}, {"name": "Bob"}]


def test_execute_sql_without_rows(server: FlowmaticDBMCP) -> None:
    assert server.execute_sql("CREATE TABLE things (id INTEGER)") == []


def test_list_tables(server: FlowmaticDBMCP) -> None:
    assert server.list_tables() == ["users"]


def test_describe_table(server: FlowmaticDBMCP) -> None:
...
def test_describe_table_reports_constraints(server: FlowmaticDBMCP) -> None:
...
def test_select_without_wheres(server: FlowmaticDBMCP) -> None:
    assert names(server.select("users")) == ["Alice", "Bob", "Cara"]


def test_select_limit_and_offset(server: FlowmaticDBMCP) -> None:
    assert names(server.select("users", None, None, None, 2, 1)) == ["Bob", "Cara"]


def test_select_group_by_collapses_rows(grouped: FlowmaticDBMCP) -> None:
    rows = grouped.select("orders", None, ["customer"])

    assert [row["customer"] for row in rows] == ["ann", "bob", "cid"]


def test_select_group_by_a_qualified_column(grouped: FlowmaticDBMCP) -> None:
    rows = grouped.select("orders", None, [["orders", "customer"]])

    assert [row["customer"] for row in rows] == ["ann", "bob", "cid"]


def test_select_havings_filter_the_groups(grouped: FlowmaticDBMCP) -> None:
...
def test_select_havings_are_chained_with_and(grouped: FlowmaticDBMCP) -> None:
    havings = [
...
def test_having_operators(grouped: FlowmaticDBMCP, operator: str, value: Any, expected: list[str]) -> None:
...
def test_having_range_operators(grouped: FlowmaticDBMCP) -> None:
...
def test_wheres_and_havings_combine(grouped: FlowmaticDBMCP) -> None:
...
def test_unknown_having_operator_is_refused(grouped: FlowmaticDBMCP) -> None:
...
def test_having_in_needs_a_list(grouped: FlowmaticDBMCP) -> None:
...
def test_group_by_and_havings_over_the_protocol(grouped: FlowmaticDBMCP) -> None:
    arguments = {
        "table": "orders",
        "group_by": ["customer"],
...
def test_select_wheres_are_chained_with_and(server: FlowmaticDBMCP) -> None:
    wheres = [
...
def test_name_operators(server: FlowmaticDBMCP, operator: str, value: Any, expected: list[str]) -> None:
...
def test_rollback_transaction(server: FlowmaticDBMCP) -> None:
...
def test_transactions_do_not_nest_implicitly(server: FlowmaticDBMCP) -> None:
    server.begin_transaction()
...
def test_commit_without_a_transaction(server: FlowmaticDBMCP) -> None:
    with pytest.raises(QueryError, match="no transaction is open"):
        server.commit_transaction()


def test_rollback_without_a_transaction(server: FlowmaticDBMCP) -> None:
    with pytest.raises(QueryError, match="no transaction is open"):
        server.rollback_transaction()


def test_rollback_savepoint_keeps_the_transaction_open(server: FlowmaticDBMCP) -> None:
    server.begin_transaction()
    server.insert("users", [{"name": "Dan", "age": 55}])
...
def test_commit_savepoint_folds_into_the_transaction(server: FlowmaticDBMCP) -> None:
    server.begin_transaction()
...
def test_savepoints_nest(server: FlowmaticDBMCP) -> None:
    server.begin_transaction()
...
def test_savepoint_needs_a_transaction(server: FlowmaticDBMCP) -> None:
    with pytest.raises(QueryError, match="no transaction is open"):
        server.begin_savepoint("sp")


def test_savepoint_names_are_unique(server: FlowmaticDBMCP) -> None:
    server.begin_transaction()
    server.begin_savepoint("sp")
...
def test_savepoints_close_innermost_first(server: FlowmaticDBMCP) -> None:
    server.begin_transaction()
    server.begin_savepoint("outer")
...
def test_closing_a_savepoint_that_is_not_open(server: FlowmaticDBMCP) -> None:
    server.begin_transaction()
...
def test_committing_a_transaction_releases_its_savepoints(server: FlowmaticDBMCP) -> None:
    server.begin_transaction()
    server.begin_savepoint("sp")
    server.insert("users", [{"name": "Dan", "age": 55}])
    assert server.commit_transaction() == {"in_transaction": False, "savepoints": []}
...
def test_json_and_datetime_values_are_json_safe(server: FlowmaticDBMCP) -> None:
...
def test_text_in_a_binary_column_stays_readable(server: FlowmaticDBMCP) -> None:
...
def test_binary_that_is_not_text_is_base64_encoded(server: FlowmaticDBMCP) -> None:
    server.db.exec("CREATE TABLE blobs (id INTEGER PRIMARY KEY, payload BLOB)")
