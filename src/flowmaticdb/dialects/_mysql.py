...
class MySQLDialect(SQLDialect):
    ...
    def _build_column(self, col: Column) -> str:
        # The base builder emits no keyword for auto-increment columns because
        # MySQL has no GENERATED BY DEFAULT AS IDENTITY; it only drops the
        # default, which MySQL rejects here too.
    ...
    def _build_column_default(self, default: Any, sql_type: str) -> str:
        # MySQL rejects DEFAULT CURRENT_TIMESTAMP (and its synonym NOW()) on a
        # DATETIME/TIMESTAMP column that has a fractional seconds part: the
        # precision of the default must match the column's.
