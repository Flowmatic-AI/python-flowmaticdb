...
| Command | Purpose |
|---------|---------|
| `python3 -m pytest` | Run all tests (unit + SQLite integration; 451 passing without a server, 480 with MySQL and PostgreSQL up) |
...
- **Testing**: 451 tests pass without any database (unit + SQLite in-memory/file integration, including `test_threading.py`); 480 with both servers up.
